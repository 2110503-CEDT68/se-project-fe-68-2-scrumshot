...
export default function BookingDetailPage({
      ...
      if (bookingRes.success) {
        ...
        if (!isEditMode) {
          setSelectedCampgroundId(
            typeof data.campground === "object"
              ? (data.campground as any)._id
              : data.campground,
          );
          setUserId(data.user);
  ...
  const campgroundPricePerNight =
    campgrounds.find((c: Campground) => c._id === selectedCampgroundId)
      ?.pricePerNight || 0;
  ...
  useEffect(() => {
    ...
    if (isEditMode && bookDate && bookEndDate && campgroundPricePerNight) {
      ...
      const calculated =
        nights > 0 ? (nights * campgroundPricePerNight).toString() : "0";
  ...
  const toggleEditMode = () => {
    if (isEditMode && originalBooking) {
      setSelectedCampgroundId(
        typeof originalBooking.campground === "object"
          ? (originalBooking.campground as any)._id
          : originalBooking.campground,
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isAdmin && isManualPrice) {
        const resDate = await updateBooking(
          bookingId,
        ...
        const resPrice = await updateBooking(
          bookingId,
          session.user.backendToken,
          {
        ...
        <div className="space-y-6 max-w-2xl mx-auto">
          {isAdmin && isEditMode && (
            <div className="grid grid-cols-4 items-center gap-4">
              <label className="font-semibold text-lg text-red-600">
                Owner User ID:
              </label>
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="col-span-3">
              {isEditMode ? (
                ...
                campgrounds.find((c) => c._id === selectedCampgroundId)?.name ||
                "Loading..."
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="col-span-3">
              {isEditMode ? (
                <DateReserve
                  value={bookDate}
                  onDateChange={(val) => setBookDate(val)}
                />
              ) : (
                bookDate?.format("DD-MM-YYYY")
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="col-span-3">
              {isEditMode ? (
                <DateReserve
                  value={bookEndDate}
                  onDateChange={(val) => setBookEndDate(val)}
                />
              ) : (
                bookEndDate?.format("DD-MM-YYYY")
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="col-span-3">
              {isAdmin && isEditMode && isManualPrice ? (
                ...
                <span
                  className={`text-xl font-bold ${isManualPrice ? "text-purple-600" : "text-black"}`}
                >
                  {Number(manualTotalCost).toLocaleString()} THB
                  {isEditMode && !isManualPrice && (
          ...
          {!isEditMode ? (
            <>
              ...
              <Button
                ...
                variant="contained"
                className="bg-[#6750A4] text-white px-6 py-2 rounded-md font-semibold text-lg"
              >
                EDIT
              </Button>
            </>
            ...
            <>
              <Button
                ...
                variant="contained"
                color="warning"
                className="px-6 py-2 rounded-md font-semibold text-lg"
              >
                CANCEL
              </Button>
              <Button
                ...
                variant="contained"
                color="primary"
                disabled={Number(manualTotalCost) < 0 || isUpdating}
                className="px-6 py-2 rounded-md font-semibold text-lg"
              >
